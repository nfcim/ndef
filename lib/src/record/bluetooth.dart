...
class DeviceClass{
  ...
  static String getServiceClassName(int index){
    assert(index>=13 && index<24,"Index of Service Class Name must be in [13,24)");
...
class EIR {
  ...
  static const Map<EIRType,String> typeNameMap={
    ...
    EIRType.LESecureConnectionsConfirmationValue : "LE Secure Connections Confirmation Value",
    EIRType.LESecureConnectionsRandomValue : "LE Secure Connections Random Value",
    EIRType.URI : "URI",
...
class BluetoothRecord extends Record{
  ...
  BluetoothRecord({Map<int,Uint8List> attributes}){
    ...
    this.attributes = attributes==null ? new Map<EIRType,Uint8List>() : attributes;
  }
  ...
  BigInt getIntValue(EIRType type){
    return ByteStream.list2bigInt(attributes[type],endianness: Endianness.little);
  }

  void setIntValue(EIRType type, BigInt value){
    setAttribute(type, ByteStream.bigInt2list(value, 16, endianness: Endianness.little));
  }
...
class BluetoothEasyPairingRecord extends BluetoothRecord{
  ...
  Uint8List get payload{
    ...
    var payload=ByteStream.int2list(data.length,2,endianness: Endianness.little)+address.bytes+data;
    return payload;
  }
...
class BluetoothLowEnergyRecord extends BluetoothRecord{
  ...
  get address{
    if(attributes.containsKey(EIRType.LEBluetoothDeviceAddress)){
      return new Address.fromBytes(attributes[EIRType.LEBluetoothDeviceAddress]);
    }else{
      return null;
    }
  }
  ...
  get roleCapabilities{
    if(attributes.containsKey(EIRType.LERole)){
      assert(attributes[EIRType.LERole].length==1,"Bytes length of LE Role must be 1");
  ...
  get appearance{
    if(attributes.containsKey(EIRType.Appearance)){
      assert(attributes[EIRType.Appearance].length==4,"Bytes length of appearance must be 4");
      int value = ByteStream.list2int(attributes[EIRType.Appearance],endianness: Endianness.little);
  ...
  set appearance(String appearance){
    ...
    attributes[EIRType.Appearance]=ByteStream.int2list(index, 4,endianness: Endianness.little);
  }
